...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	if err := src.Handshake(1, handler.chain, eth.BlockRangeUpdatePacket{}, td); err != nil {
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	if err := sink.Handshake(1, handler.chain, eth.BlockRangeUpdatePacket{}, td); err != nil {
		t.Fatalf("failed to run protocol handshake")
	}
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		if err := sinkPeer.Handshake(1, source.chain, eth.BlockRangeUpdatePacket{}, td); err != nil {
			t.Fatalf("failed to run protocol handshake")
		}
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	if err := sink.Handshake(1, source.chain, eth.BlockRangeUpdatePacket{}, td); err != nil {
		t.Fatalf("failed to run protocol handshake")
	}
	// After the handshake completes, the source handler should stream the sink
